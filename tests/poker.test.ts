...
describe('Генераторы и сохранение',()=>{
 it('все типы дают допустимые карты и проверяемые ответы',()=>{for(const s of allSkills)for(let i=0;i<25;i++){const t=generate(s);validate([...(t.cards??[]),...(t.board??[]),...(t.opponent??[])]);expect(new Set(t.choices).size).toBe(t.choices.length);expect(t.choices).toContain(t.answer);expect(check(t,t.answer,t.selection?evaluate(t.cards!).best:[])).toBe(true);}},20000);
 it('принимает альтернативную лучшую пятёрку, не принимает чужие карты',()=>{const t={...generate('best'),cards:cards('As Ah Kc Qc Jc Tc 2d')};expect(check(t,'',cards('Ah Kc Qc Jc Tc'))).toBe(true);expect(check(t,'',cards('Ad Kc Qc Jc Tc'))).toBe(false);});
